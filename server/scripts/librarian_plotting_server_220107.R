...
read_tsv("../data/test_library_composition_220107.txt", col_names = F) -> test.import
...
types <- c("ATAC-Seq","Bisulfite-Seq", "ChIA-PET","ChIP-Seq", "DNase-Hypersensitivity", "Hi-C", "MBD-Seq", "MeDIP-Seq", "miRNA-Seq", "MNase-Seq", "ncRNA-Seq","RIP-Seq", "RNA-Seq", "ssRNA-Seq")
...
umap_grid_tile_long %>% 
  ...
  theme(aspect.ratio = 0.8) -> umap_grid_tile_plot

print(umap_grid_tile_plot)
...
grid_tile_corr_percentages %>% 
  filter(grid_ID == test_ID) %>% 
  select(-grid_ID) %>% 
  pivot_longer(1:last_col(), names_to = "library_type", values_to = "percent") %>% 
  mutate(library_type = fct_reorder(library_type, percent, .fun = min)) %>% 
  arrange(library_type) -> test_percentage
...
test_percentage %>% 
  ...
  geom_col() +
  scale_fill_manual(values = test_percentage %>% 
                      left_join(colour_mapping) %>% 
                      pull(colours)) +
  coord_flip() +
  theme_bw(base_size = 14) +
  theme(axis.title = element_blank(), legend.position = "none") -> test_percentage_plot
