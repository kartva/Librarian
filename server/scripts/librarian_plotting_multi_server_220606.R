## This script reads in the base compositions of the 2018, 2019, 2020 GEO search which is filtered for the most frequent library types and limits the number of examples 
...
#svg(file=file.path(args[2], "graph_%d.svg"), width = 7, height = 7)
...
#read_tsv("../data/test_library_composition_5_220304.txt", col_names = F) -> test.import
#read_tsv("../data/test_library_composition_1_220304.txt", col_names = F) -> test.import
...
output %>% 
  ...
  distinct(srr_number, .keep_all = T) %>% 
  mutate(lib_type = gsub("DNase-Hypersensitivity", "DNase-HS", lib_type)) %>%
  mutate(lib_type = gsub("ATAC-seq", "ATAC-Seq", lib_type)) %>%
  mutate(lib_type = gsub("ssRNA-seq", "ssRNA-Seq", lib_type)) -> compositions # deduplicating SRR numbers (I checked, some are duplicated, they are carried through from 
...
compositions_umap <- umap(umap.data, n_neighbors = 15, min_dist = 0.8) 
...
types <- c("ATAC-Seq","Bisulfite-Seq", "ChIA-PET","ChIP-Seq", "DNase-HS", "Hi-C", "MBD-Seq", "MeDIP-Seq", "miRNA-Seq", "MNase-Seq", "ncRNA-Seq","RIP-Seq", "RNA-Seq", "ssRNA-Seq")
...
## Rasterising the point layer of the plot
...
## Rasterising the tile layer of the plot
...
#print(umap_grid_tile_long_plot)
...
test_percentage %>% 
  ...
  #ggplot(aes(fct_rev(sample), percent, fill = reorder(library_type, percent))) +
  geom_col() +
...
ggsave(filename = file.path(args[2],"Prediction_plot.svg"), width = 7, height = (3.5 + (sample_number * 0.1)), units = "in")

#print(test_percentage_plot)
